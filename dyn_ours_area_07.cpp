...
void calculate_costs(const vector<int> &banks, float &perf, float &energy);
int get_accesses(int i, int j);

int main() {
   
    //1.Set up the file to hold testing results:
    std::ostringstream s;
	s << "area_optimization_results.xls";
	string file = s.str();
	...
	{
		cout << "file created\n";
		dst << "\nTesting results optimized for: AREA\n";
		dst << "\nGran. (B)\tMin. Part. Size (B)\tEnergy Cost (nJ)\tPerformance Cost (ns)\tArea Cost (mm2) [opt.]\tCPU Time (sec)\tRAM Usage (B)\tBanking Configuration (Cut Set)\n";
	}
	else
	{
   ...
   /*for(int min_bank_size=512; min_bank_size<=4096; min_bank_size*=2)
	...
	for(granularity=4; granularity<=512; granularity*=2)
	{
	   ...
	   {			
		   ...
		   float perf_cost=0, energy_cost=0;
		   ...
		   for(j=min_bank_size/granularity; j<=N; j++)
			   for(i=1,k=j; k<=N ;i++,k++)
			   {
				   ...
				   if(k-i+1 == 0) continue;
					...
					P[i][k] = area_costs[(k-i+1)*granularity];  /*cost per access to a memory of this size (i-j) / f[i][j]; */
			   ...
			   calculate_costs(banks, perf_cost, energy_cost);
...
void calculate_costs(const vector<int> &banks, float &perf, float &energy)
{
	...
	for(i=1;(banks[i]!= 0)&&(i<banks.size()); i++)
	{
		...
		accesses = get_accesses(banks[i-1]/granularity+1, banks[i]/granularity);
		perf += accesses * performance_costs[size];
...
{
	int accesses = 0;
   for (int k=(i-1)*granularity; k<=j*granularity; k++) accesses += A[k];
   return accesses;
}