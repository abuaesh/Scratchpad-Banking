...
void calculate_costs(const vector<int> &banks, const vector<vector<int>> &f, float &perf, float &area);

int main() {
   
    //1.Set up the file to hold testing results:
    std::ostringstream s;
	s << "energy_optimization_results.xls";
	string file = s.str();
	...
	{
		cout << "file created\n";
		dst << "\nTesting results optimized for: ENERGY\n";
		dst << "\nGran. (B)\tMin. Part. Size (B)\tEnergy Cost (nJ) [opt.]\tPerformance Cost (ns)\tArea Cost (mm2)\tCPU Time (sec)\tRAM Usage (B)\tBanking Configuration (Cut Set)\n";
	}
	else
	{
		   ...
		   for(j=min_bank_size/granularity; j<=N; j++)
			   for(i=1,k=j; k<=N ;i++,k++)
			   {
				   ...
				   if(k-i+1 == 0) continue;
					...
					P[i][k] = f[i][k]*energy_costs[(k-i+1)*granularity];  /*cost per access to a memory of this size (i-j) / f[i][j]; */
